...
class Downsample:
    ...
    def __call__(self,imgs):
        features = []
        fine = imgs
        half = F.interpolate(fine, scale_factor=0.5, mode='nearest')
        coarse = F.interpolate(half, scale_factor=0.5, mode='nearest')
        features.append(fine)
        features.append(half)
        features.append(coarse)
        return features
class ReconNet(nn.Module):
    def __init__(self,fusion = True, direct = None,st = None,):
        super(ReconNet, self).__init__()
        if direct == True:
            self.feature_extraction = Downsample()
            in_channels = [4, 102, 54]
            channels = [96, 48, 24]
        else:
            if st == True:
                self.feature_extraction = SwinTransformer()
                in_channels = [97, 147, 745]
                channels = [96, 48, 24]
            else:
